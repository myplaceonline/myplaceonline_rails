...
Doorkeeper.configure do
  ...
  resource_owner_from_credentials do |_routes|
    ...
    user = request.env["warden"].authenticate!(scope: :user, store: false)
    user
  end
  ...
  admin_authenticator do
    if current_user.nil? || !current_user.admin?
      redirect_to(root_url, alert: I18n.t("myplaceonline.general.access_denied", resource: "/oauth/applications"))
    end
  end
  ...
  skip_authorization do |resource_owner, client|
    ...
    ::Rails.logger.info{"Doorkeeper: #{resource_owner}, #{client}"}
    true
  end
