class DynamicCookieOptions
  ...
  def self.cookie_domain
    ...
    if ExecutionContext.available?
      Rails.logger.info{"DynamicCookieOptions ExecutionContext available"}
      if Rails.env.production?
        domain = Myp.top_host
        Rails.logger.info{"DynamicCookieOptions domain: #{domain}"}
      end
    else
      Rails.logger.info{"DynamicCookieOptions Cookie domain incalculable #{Myp.current_stack}"}
    end
    domain
  end
