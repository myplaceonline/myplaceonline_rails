...
module ApplicationHelper
  ...
  def display_url(content:, format:, options:)
    if !content.blank?
      ...
      if !content.start_with?("/") && !content.include?(".") && !content.include?("localhost")
        content << ".com/"
      end
      ...
      while true do
        ...
        if !match_data.nil?
          ...
          if !identity_file.nil?
            ...
            replacement = ""
            Myp.warn("Image #{name_search} not found for blog: #{blog.id}, image_context: #{image_context.inspect}", request: request)
          end
  ...
  def share_token(context:, share_context: nil, valid_guest_actions: nil)
    ...
    if token.blank? || (!token.blank? && User.current_user.guest?)
      ...
      Rails.logger.debug{"ApplicationHelper.share_token: context identity = #{Ability.context_identity.inspect}, current user = #{User.current_user.current_identity_id}"}
      
      if !Ability.context_identity.nil? && Ability.context_identity.id != User.current_user.current_identity_id
      ...
      if options[:type] == Myp::FIELD_TEXT_AREA && options[:text_area_rich]
        ...
        if !options[:form].nil?
          if options[:type] == Myp::FIELD_BOOLEAN
            ...
            if options[:value].is_true? || options[:form].object.send(name.to_s).is_true?
  ...
  def header_title
    category = find_category_by_url
    if category.nil? || category.icon.blank?
      Myp.website_domain.domain_name
    else
