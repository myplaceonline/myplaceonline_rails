class MyplaceonlineController < ApplicationController
  ...
  def categoriesForCurrentUser(parent = nil, orderByName = false)
    ...
    #
    # However, this places the where clause at the end instead of as an addition to the LEFT OUTER JOIN clause, so we only get categories if the identity has points for them.
    # 
    # Instead we could do a joins() call with an explicit LEFT OUTER JOIN and the additional identity condition, but then we don't eager load the point amounts, and
    # if we go and get the point amounts, it won't go with the identity condition.
    #
    # Therefore we have to fallback to direct SQL

    Category.find_by_sql(%{
      SELECT
        #{Category.new.attributes.keys.map{|col| "categories." + col + " AS " + col}.join(", ")},
        category_points_amounts.count as points_amount
      FROM categories
      ...
      ORDER BY #{ orderByName ? "categories.name ASC" : "categories.position ASC" }
    }).map{ |category|
