...
function form_add_item(link, namePrefix, deletePlaceholder, items, singletonMessage, nonIndexBased) {
  ...
  for (i = 0; i < items.length; i++) {
    ...
    if (item.type == "date") {
      ...
      html += "<p><input type='date' id='" + id + "' name='" + name + "' placeholder='" + item.placeholder + "' value='" + defaultValue + "' class='" + cssclasses + "' data-role='datebox' data-datebox-mode='calbox' data-datebox-override-date-format='%Y-%m-%d' data-datebox-use-focus='true' data-datebox-use-clear-button='true' data-datebox-use-modal='false' /></p>";
